function drawPitRoundMap() {
    ...
    for (var i = 0; i < dataArray.length - 1; i++) {
        ...
        if (uniqueConstructorIds.indexOf(currentConstructorId) >= 0) {
            ...
            const coordinateKey = `${x}_${currentConstructorId}`;
            if (!pointCounter[coordinateKey]) {
                pointCounter[coordinateKey] = {
                    x: x, count: 1, color: dataArray[i].color, constructor: dataArray[i].name,
    ...
    Object.keys(pointCounter).forEach(key => {
        ...
        let currentYPos = lapYPositions.get(point.lap) || (stageHeight - 100);
        for (var j = 0; j < dotCount; j++) {

            // y verschieben
            yPos = currentYPos - j * 8;
            ...
            dot.hover(
                function () {
                    ...
                    durationDiv.text(`${dotData.lap}' lap`);
        ...
        lapYPositions.set(point.lap, currentYPos - dotCount * 8);
    });
}
function createElements() {
    ...
    for (var i = 0; i < dataArray.length - 1; i++) {
        ...
        if (uniqueConstructorIds.indexOf(currentConstructorId) >= 0 && currentPitDuration > 12.5) {
            var x = gmynd.map(fastestPitArray.indexOf(currentConstructorId), 0, totalUniqueValues - 1, 150, stageWidth - 150);
            var y = gmynd.map(currentPitDuration, 12.0, 59.0, stageHeight - 120, 170);

            xAverage = gmynd.map(averagePitArray.indexOf(currentConstructorId), 0, totalUniqueValues - 1, 150, stageWidth - 150);
            ...
            if (!pointCounter[coordinateKey]) {
                pointCounter[coordinateKey] = {
                    xfastest: x, xAverage: xAverage, y: y, count: 1, color: dataArray[i].color, constructor: dataArray[i].name,
    ...
    Object.keys(pointCounter).forEach(key => {
        ...
        for (var j = 0; j < dotCount; j++) {
            ...
            // x verschieben
            xPosFastest = point.xfastest + (j - offset) * 8;
            xPosAverage = point.xAverage + (j - offset) * 8;
            yLapPos = point.yLap - (j * 8);

            // xPositions.push({ pointIndex: j, duration: point.duration, constructorId: point.constructorId, xPos: xPos, xPosAverage: xPosAverage});

            let dot = $('<div class="dot"></div>');
            dot.addClass('persist');

            dot.css({
                'left': xPosFastest - 6 / 2,
                'top': point.y - 6 / 2,
                'height': dotSize,
                'width': dotSize,
                'border-radius': '50%',
            ...
            dot.data({
                ...
                xAverage: xPosAverage,
                xfastest: xPosFastest,
                xLap: point.xLap,
                yLap: yLapPos,
                yNormal: point.y,
            })
            ...
            dot.hover(
                function () {
                    ...
                    if (point.count > 3) {
                        tooltip.html(`<span style="font-weight: bold;">${dotData.constructor}</span> 
                        <span style="font-weight: lighter; margin-left: 10px;">5 PitStops</span>`);
                        // <span style="font-weight: lighter; margin-left: 10px;">${dotData.dotCount} PitStops</span>`);
                    }
